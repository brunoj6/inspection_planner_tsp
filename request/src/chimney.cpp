...
int main(int argc, char **argv)
{
  ...
  */
  ...
  std::vector<nav_msgs::Path> * mesh = readSTLfile(ros::package::getPath("request")+"/meshes/chimney.stl");
  ...
  if (client.call(srv))
  {
    ...
    // stop timer
    ROS_INFO("Planning took %f s", (ros::Time::now() - begin).toSec());
  ...
  while(0 != strcmp(strtok(line, " "), "endsolid") && !ros::isShuttingDown())
  {
    ...
    for(int i = 0; i<7; i++)
    {
      ...
      if(line[q] == 'v')
      {
        ...
        const double scaleFactor = 10;   // used to scale the mesh before processing
